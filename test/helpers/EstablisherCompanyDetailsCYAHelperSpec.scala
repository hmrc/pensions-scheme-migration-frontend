...
import utils.Data.{company, credId, psaId, pstr, schemeName}
import utils.{Enumerable, UserAnswers}
import viewmodels.Message

class EstablisherCompanyDetailsCYAHelperSpec extends WordSpec with MustMatchers with TryValues with Enumerable.Implicits {
  ...
  "EstablisherCompanyDetailsCYAHelper" must {
    "return all rows with correct change links, values and visually hidden text when user has answered yes for all questions" in {
      ...
      result.head mustBe Row(
        key = Key(msg"${Message("messages__haveCompanyNumber", company.companyName).resolve}", classes = Seq("govuk-!-width-one-half")),
        ...
        actions = Seq(Action(
          ...
          visuallyHiddenText = Some(Literal(Messages("site.change") + " " +
            Messages("messages__haveCompanyNumber__cya__visuallyHidden", company.companyName))),
      ...
      result(1) mustBe Row(
        ...
        actions = Seq(Action(
          ...
          visuallyHiddenText = Some(Literal(Messages("site.change") + " " +
            Messages("messages__companyNumber__cya__visuallyHidden", company.companyName))),
          attributes = Map("id" -> "cya-0-1-change")
        ))
      )

      result(2) mustBe Row(
        key = Key(msg"${Message("messages__hasUTR", company.companyName).resolve}", classes = Seq("govuk-!-width-one-half")),
        value = Value(Literal("Yes")),
        actions = Seq(Action(
          content = Html(s"<span aria-hidden=true >${messages("site.change")}</span>"),
          href = routes.HaveUTRController.onPageLoad(0, CheckMode).url,
          visuallyHiddenText = Some(Literal(Messages("site.change") + " " +
            Messages("messages__hasUTR__cya__visuallyHidden", company.companyName))),
      ...
      result(3) mustBe Row(
        ...
        actions = Seq(Action(
          ...
          visuallyHiddenText = Some(Literal(Messages("site.change") + " " +
            Messages("messages__enterUTR__cya__visuallyHidden", company.companyName))),
          attributes = Map("id" -> "cya-0-3-change")
        ))
      )

      result(4) mustBe Row(
        key = Key(msg"${Message("messages__haveVAT", company.companyName).resolve}", classes = Seq("govuk-!-width-one-half")),
        value = Value(Literal("Yes")),
        actions = Seq(Action(
          content = Html(s"<span aria-hidden=true >${messages("site.change")}</span>"),
          href = routes.HaveVATController.onPageLoad(0, CheckMode).url,
          visuallyHiddenText = Some(Literal(Messages("site.change") + " " +
            Messages("messages__haveVAT__cya__visuallyHidden", company.companyName))),
      ...
      result(5) mustBe Row(
        ...
        actions = Seq(Action(
          ...
          visuallyHiddenText = Some(Literal(Messages("site.change") + " " +
            Messages("messages__vat__cya__visuallyHidden", company.companyName))),
          attributes = Map("id" -> "cya-0-5-change")
        ))
      )

      result(6) mustBe Row(
        key = Key(msg"${Message("messages__havePAYE", company.companyName).resolve}", classes = Seq("govuk-!-width-one-half")),
        value = Value(Literal("Yes")),
        actions = Seq(Action(
          content = Html(s"<span aria-hidden=true >${messages("site.change")}</span>"),
          href = routes.HavePAYEController.onPageLoad(0, CheckMode).url,
          visuallyHiddenText = Some(Literal(Messages("site.change") + " " +
            Messages("messages__havePAYE__cya__visuallyHidden", company.companyName))),
      ...
      result(7) mustBe Row(
        ...
        actions = Seq(Action(
          ...
          visuallyHiddenText = Some(Literal(Messages("site.change") + " " +
            Messages("messages__paye__cya__visuallyHidden", company.companyName))),
    ...
    "return all rows with correct change links, values and visually hidden text when user has answered no for all questions" in {
      ...
      result(1) mustBe Row(
        key = Key(msg"${Message("messages__whyNoCompanyNumber", company.companyName).resolve}", classes = Seq("govuk-!-width-one-half")),
        value = Value(Literal("reason-company-number")),
        actions = Seq(Action(
          content = Html(s"<span aria-hidden=true >${messages("site.change")}</span>"),
          href = routes.NoCompanyNumberReasonController.onPageLoad(0, CheckMode).url,
          visuallyHiddenText = Some(Literal(Messages("site.change") + " " +
            Messages("messages__whyNoCompanyNumber__cya__visuallyHidden", company.companyName))),
          attributes = Map("id" -> "cya-0-1-change")
        ))
      )
      ...
      result(3) mustBe Row(
        key = Key(msg"${Message("messages__whyNoUTR", company.companyName).resolve}", classes = Seq("govuk-!-width-one-half")),
        value = Value(Literal("reason-utr")),
        actions = Seq(Action(
          content = Html(s"<span aria-hidden=true >${messages("site.change")}</span>"),
          href = routes.NoUTRReasonController.onPageLoad(0, CheckMode).url,
          visuallyHiddenText = Some(Literal(Messages("site.change") + " " +
            Messages("messages__whyNoUTR__cya__visuallyHidden", company.companyName))),
          attributes = Map("id" -> "cya-0-3-change")
        ))
      )
