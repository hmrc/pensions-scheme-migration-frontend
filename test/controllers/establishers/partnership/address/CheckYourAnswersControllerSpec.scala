...
class CheckYourAnswersControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers {
  ...
  private val mockCyaHelper: EstablisherPartnershipAddressCYAHelper = mock[EstablisherPartnershipAddressCYAHelper]
  private def httpPathGET: String = controllers.establishers.partnership.address.routes.CheckYourAnswersController.onPageLoad(0).url
  val extraModules: Seq[GuiceableModule] = Seq(
    bind[EstablisherPartnershipAddressCYAHelper].toInstance(mockCyaHelper)
  )
