...
class PartnerNoUTRReasonControllerSpec
  ...
  private val pageTitle = "Why does the partner not have a UTR?"
  ...
  private val httpPathGET = controllers.establishers.partnership.partner.details.routes
    .PartnerNoUTRReasonController.onPageLoad(establisherIndex, partnerIndex, mode).url
  private val httpPathPOST = controllers.establishers.partnership.partner.details.routes
    .PartnerNoUTRReasonController.onSubmit(establisherIndex, partnerIndex, mode).url
  ...
  "PartnerNoUTRReasonController" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val ua = userAnswers.set(PartnerNoUTRReasonId(0,0), formData).success.value
