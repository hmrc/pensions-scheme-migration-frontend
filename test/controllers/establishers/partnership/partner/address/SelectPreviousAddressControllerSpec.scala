...
class SelectPreviousAddressControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers with Enumerable.Implicits {
  ...
  private val httpPathGET: String = controllers.establishers.partnership.partner.address.routes.SelectPreviousAddressController.onPageLoad(0, 0, NormalMode).url
  private val httpPathPOST: String = controllers.establishers.partnership.partner.address.routes.SelectPreviousAddressController.onSubmit(0, 0, NormalMode).url
  ...
  "SelectPreviousAddress Controller" must {
    ...
    "Return OK and the correct view for a GET" in {
      val ua: UserAnswers = Data.ua
        .setOrException(EnterPreviousPostCodeId(0, 0), seqAddresses)
