...
class PartnerNameControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers with Enumerable.Implicits {
  ...
  private def httpPathGET: String = controllers.establishers.partnership.partner.routes.PartnerNameController.onPageLoad(index,partnerIndex,NormalMode).url
  private def httpPathPOST: String = controllers.establishers.partnership.partner.routes.PartnerNameController.onSubmit(index,partnerIndex,NormalMode).url
