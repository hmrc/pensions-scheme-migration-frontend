...
class ConfirmAddressControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers with Enumerable.Implicits {
  ...
  private val httpPathGET: String = controllers.establishers.individual.address.routes.ConfirmAddressController.onPageLoad(0,NormalMode).url
  private val httpPathPOST: String = controllers.establishers.individual.address.routes.ConfirmAddressController.onPageLoad(0,NormalMode).url
  ...
  "ConfirmAddress Controller" must {
    ...
    "Save data to user answers and redirect to next page when valid data is submitted" in {
      ...
      (jsonCaptor.getValue \ "establishers" \ 0 \ AddressId.toString).get mustBe expectedJson
    }
