...
class ConfirmPreviousAddressControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers with Enumerable.Implicits {
  ...
  private val ua: UserAnswers =
    Data.ua.setOrException(EstablisherNameId(0), Data.individualName)

  private val userAnswers: Option[UserAnswers] = Some(ua)
  private val mutableFakeDataRetrievalAction: MutableFakeDataRetrievalAction = new MutableFakeDataRetrievalAction()
  private val application: Application = applicationBuilderMutableRetrievalAction(mutableFakeDataRetrievalAction, extraModules).build()
  private val httpPathGET: String = controllers.establishers.individual.address.routes.ConfirmPreviousAddressController.onPageLoad(0,NormalMode).url
  private val httpPathPOST: String = controllers.establishers.individual.address.routes.ConfirmPreviousAddressController.onSubmit(0,NormalMode).url
  ...
  "ConfirmPreviousAddress Controller" must {
    ...
    "Return OK and the correct view for a GET" in {
      val ua: UserAnswers = UserAnswers()
        ...
        .setOrException(EstablisherNameId(0), Data.individualName)
    ...
    "Save data to user answers and redirect to next page when valid data is submitted" in {
      ...
      (jsonCaptor.getValue \ "establishers" \ 0 \ PreviousAddressId.toString).get mustBe expectedJson
    }
