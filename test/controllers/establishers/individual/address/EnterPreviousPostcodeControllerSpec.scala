...
class EnterPreviousPostcodeControllerSpec extends ControllerSpecBase with JsonMatchers with Enumerable.Implicits {
  ...
  private val form = formProvider("individualEnterPreviousPostcode.required", "enterPostcode.invalid")
  private val mode = NormalMode
  private val index = 0

  private val userAnswers: Option[UserAnswers] = Some(ua)
  private val httpPathGET: String = controllers.establishers.individual.address.routes.EnterPreviousPostcodeController.onPageLoad(index, mode).url
  private val httpPathPOST: String = controllers.establishers.individual.address.routes.EnterPreviousPostcodeController.onSubmit(index, mode).url
