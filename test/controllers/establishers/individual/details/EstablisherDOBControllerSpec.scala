...
class EstablisherDOBControllerSpec
  ...
  private val templateToBeRendered: String =
    "dob.njk"
  private val commonJson: Form[LocalDate] => JsObject =
    form => Json.obj(
      "form"       -> form,
      "name"       -> "Jane Doe",
  ...
  "EstablisherDOBController" must {
    "return OK and the correct view for a GET" in {
      ...
      jsonCaptor.getValue must containJson(commonJson.apply(form) ++ json)
    }
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      jsonCaptor.getValue must containJson(commonJson.apply(form.fill(formData)) ++ json)
    }
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      ...
      val result: Future[Result] =
        controller(getData)
          .onSubmit(0, NormalMode)(fakeRequest.withFormUrlEncodedBody(("date", "invalid value")))
      ...
      jsonCaptor.getValue must containJson(commonJson.apply(boundForm) ++ json)
    }
  }
}
