...
class EstablisherNoUTRReasonControllerSpec
  ...
  private val httpPathGET = controllers.establishers.individual.details.routes.EstablisherNoUTRReasonController.onPageLoad(index, mode).url
  private val httpPathPOST = controllers.establishers.individual.details.routes.EstablisherNoUTRReasonController.onSubmit(index, mode).url
  ...
  "EstablisherNoUTRReasonController" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val ua = userAnswers.set(EstablisherNoUTRReasonId(0), formData).success.value
