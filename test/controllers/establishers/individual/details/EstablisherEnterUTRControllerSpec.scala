...
import utils.Data.{individualName, schemeName, ua}
  ...
  "EstablisherEnterUTRController" must {
    "return OK and the correct view for a GET" in {
      ...
      val view = app.injector.instanceOf[EnterReferenceValueWithHintView].apply(
        ...
        submitCall= routes.EstablisherEnterUTRController.onSubmit(0, NormalMode)
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      ...
      contentAsString(result) must include(messages("messages__enterUTR", "the individual"))
      contentAsString(result) must include(messages("messages__utr__error_invalid", individualName.fullName))
