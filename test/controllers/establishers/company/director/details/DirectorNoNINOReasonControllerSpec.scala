...
class DirectorNoNINOReasonControllerSpec
  ...
  private val commonJson: JsObject =
    Json.obj(
      "pageTitle"     -> "Why does the director not have a National Insurance number?",
  ...
  private def controller(
                        ...
                        ): DirectorNoNINOReasonController =
  ...
  "DirectorNoNINOReasonController" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val ua =
        userAnswers
          .set(DirectorNoNINOReasonId(0,0), formData).success.value
