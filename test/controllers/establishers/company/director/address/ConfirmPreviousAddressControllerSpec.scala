...
class ConfirmPreviousAddressControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers with Enumerable.Implicits {
  ...
  private val httpPathGET: String = controllers.establishers.company.director.address.routes.ConfirmPreviousAddressController.onPageLoad(0, 0, NormalMode).url
  private val httpPathPOST: String = controllers.establishers.company.director.address.routes.ConfirmPreviousAddressController.onSubmit(0, 0, NormalMode).url
  ...
  "ConfirmPreviousAddress Controller" must {
    ...
    "Save data to user answers and redirect to next page when valid data is submitted" in {
      ...
      (jsonCaptor.getValue \ "establishers" \ 0 \ "director" \ 0 \ PreviousAddressId.toString).get mustBe expectedJson
    }
