...
class SelectPreviousAddressControllerSpec extends ControllerSpecBase with JsonMatchers with Enumerable.Implicits {
  ...
  private val httpPathGET: String = controllers.establishers.company.director.address.routes.SelectPreviousAddressController.onPageLoad(index, index, mode).url
  private val httpPathPOST: String = controllers.establishers.company.director.address.routes.SelectPreviousAddressController.onSubmit(index, index, mode).url
