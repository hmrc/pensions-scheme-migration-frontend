...
class SelectAddressControllerSpec extends ControllerSpecBase with JsonMatchers with Enumerable.Implicits {
  ...
  private val httpPathGET: String = controllers.establishers.company.director.address.routes.SelectAddressController.onPageLoad(index, index, mode).url
  private val httpPathPOST: String = controllers.establishers.company.director.address.routes.SelectAddressController.onSubmit(index, index, mode).url
