...
class HaveCompanyNumberControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers with TryValues with BeforeAndAfterEach {
  ...
  private val form: Form[Boolean] = formProvider("Select Yes if Jane Doe has a Unique Taxpayer Reference")
  ...
  private def controller(
                        ...
                        ): HaveCompanyNumberController =
    new HaveCompanyNumberController(
  ...
  "EstablisherHasUTRController" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val ua = userAnswers.set(EstablisherHasUTRId(0), true).success.value
