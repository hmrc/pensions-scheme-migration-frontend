...
import identifiers.establishers.company.address.EnterPreviousPostCodeId
...
class SelectPreviousAddressControllerSpec extends ControllerSpecBase with JsonMatchers with Enumerable.Implicits {
  ...
  private val httpPathGET: String = controllers.establishers.company.address.routes.SelectPreviousAddressController.onPageLoad(index,mode).url
  private val httpPathPOST: String = controllers.establishers.company.address.routes.SelectPreviousAddressController.onSubmit(index,mode).url
  ...
  "SelectPreviousAddress Controller" must {
    ...
    "Return OK and the correct view for a GET" in {
      val ua: UserAnswers = Data.ua
        .setOrException(EnterPreviousPostCodeId(index), seqAddresses)
