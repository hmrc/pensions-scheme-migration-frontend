...
class ConfirmPreviousAddressControllerSpec extends ControllerSpecBase with JsonMatchers with Enumerable.Implicits {
  ...
  private val httpPathGET: String = controllers.establishers.company.address.routes.ConfirmPreviousAddressController.onPageLoad(index,mode).url
  private val httpPathPOST: String = controllers.establishers.company.address.routes.ConfirmPreviousAddressController.onSubmit(index,mode).url
