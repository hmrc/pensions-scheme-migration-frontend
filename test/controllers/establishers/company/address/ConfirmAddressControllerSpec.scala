...
class ConfirmAddressControllerSpec extends ControllerSpecBase with JsonMatchers with Enumerable.Implicits {
  ...
  private val httpPathGET: String = controllers.establishers.company.address.routes.ConfirmAddressController.onPageLoad(index,mode).url
  private val httpPathPOST: String = controllers.establishers.company.address.routes.ConfirmAddressController.onSubmit(index,mode).url
