...
class AddressYearsControllerSpec extends ControllerSpecBase with JsonMatchers with Enumerable.Implicits {
  ...
  private val httpPathGET: String = controllers.establishers.company.address.routes.AddressYearsController.onPageLoad(index,mode).url
  private val httpPathPOST: String = controllers.establishers.company.address.routes.AddressYearsController.onSubmit(index,mode).url
