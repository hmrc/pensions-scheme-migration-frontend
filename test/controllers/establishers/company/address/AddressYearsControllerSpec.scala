...
class AddressYearsControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers with Enumerable.Implicits {
  ...
  private val httpPathPOST: String = controllers.establishers.company.address.routes.AddressYearsController.onSubmit(0).url
  ...
  "AddressYears Controller" must {
    ...
    "Return OK and the correct view for a GET" in {
      ...
      verify(mockRenderer, times(1))
        .render(Matchers.eq("establishers/company/address/addressYears.njk"), jsonCaptor.capture())(any())
