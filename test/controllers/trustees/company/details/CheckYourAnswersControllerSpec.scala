...
class CheckYourAnswersControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers {
  ...
  private val mockCyaHelper: TrusteeCompanyDetailsCYAHelperForTwirl = mock[TrusteeCompanyDetailsCYAHelperForTwirl]
  private def httpPathGET: String = routes.CheckYourAnswersController.onPageLoad(0).url
  val extraModules: Seq[GuiceableModule] = Seq(
    bind[TrusteeCompanyDetailsCYAHelperForTwirl].toInstance(mockCyaHelper)
  )
  private val application: Application = applicationBuilderMutableRetrievalAction(mutableFakeDataRetrievalAction, extraModules).build()
  private val rows = Seq(
    SummaryListRow(
      key = Key(HtmlContent("test-key"), classes = "govuk-!-width-one-half"),
      value = Value(HtmlContent(Messages("site.incomplete")), classes = "govuk-!-width-one-third"),
      actions = Some(
        Actions(
          items = Seq(
            ActionItem(
