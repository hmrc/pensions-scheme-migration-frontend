...
class EnterPostcodeControllerSpec extends ControllerSpecBase with NunjucksSupport with JsonMatchers with Enumerable.Implicits {
  ...
  private val httpPathGET: String = controllers.adviser.routes.EnterPostcodeController.onPageLoad(NormalMode).url
  private val httpPathPOST: String = controllers.adviser.routes.EnterPostcodeController.onSubmit(NormalMode).url
  ...
  "EnterPostcode Controller" must {
    ...
    "Save data to user answers and redirect to next page when valid data is submitted" in {
      ...
      when(mockCompoundNavigator.nextPage(any(), any(), any())(any()))
        .thenReturn(routes.ConfirmAddressController.onPageLoad)
