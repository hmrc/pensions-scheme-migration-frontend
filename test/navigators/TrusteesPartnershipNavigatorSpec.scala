...
class TrusteesPartnershipNavigatorSpec extends SpecBase with NavigatorBehaviour with Enumerable.Implicits with TryValues {
  ...
  private def phone(mode: Mode): Call = controllers.trustees.partnership.contact.routes.EnterPhoneController.onPageLoad(index, mode)
  ...
  private val cyaContact: Call =
    controllers.trustees.partnership.contact.routes.CheckYourAnswersController.onPageLoad(index)
  ...
  "TrusteesPartnershipNavigator" when {
    def navigation: TableFor3[Identifier, UserAnswers, Call] =
      Table(
        ...
        row(EnterEmailId(index))(phone(NormalMode)),
