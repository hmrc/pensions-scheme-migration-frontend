...
class CommonPhoneServiceSpec extends CommonServiceSpecBase with MockitoSugar with ScalaFutures with BeforeAndAfterEach {
  ...
  val phoneView = app.injector.instanceOf[PhoneView]
  private val service = new CommonPhoneService(
    controllerComponents, renderer, mockUserAnswersCacheConnector, navigator, messagesApi, phoneView)
  ...
  "CommonPhoneService" must {
    ...
    "render the view correctly on get" in {
      ...
      val result = service.get("entityName", Message("entityType"), phoneId, form, "schemeName", submitCall=onwardCall)(request, global)
    ...
    "return a BadRequest and errors when invalid data is submitted on post" in {
      val invalidRequest: DataRequest[AnyContent] = DataRequest(FakeRequest().withFormUrlEncodedBody("value" -> ""),
        UserAnswers(Json.obj("id" -> userAnswersId)), PsaId("A2110001"), Data.migrationLock)
    ...
    "save the data and redirect correctly on post" in {
      val validRequest: DataRequest[AnyContent] = DataRequest(FakeRequest().withFormUrlEncodedBody("value" -> "1234567890"),
        UserAnswers(Json.obj("id" -> userAnswersId)), PsaId("A2110001"), Data.migrationLock)
