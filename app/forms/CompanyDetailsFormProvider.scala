...
class CompanyDetailsFormProvider @Inject() extends Mappings with Transforms {
  ...
  def apply(): Form[CompanyDetails] = Form(
    mapping(
      "companyName" -> text(errorKey = "messages__error__company_name")
        .verifying(
          firstError(
            maxLength(
              companyNameLength,
              errorKey = "messages__error__company_name_length"
            ),
            safeText(errorKey = "messages__error__company_name_invalid")
          )
        )
