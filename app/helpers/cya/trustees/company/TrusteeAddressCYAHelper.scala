  ...
  def rows(index: Index)(
    ...
    val seqRowAddressAndYears = Seq(
      answerOrAddRow(
        ...
        Some(controllers.trustees.company.address.routes.EnterPostcodeController.onPageLoad(index,CheckMode).url),
      ...
      answerOrAddRow(
        ...
        Some(controllers.trustees.company.address.routes.AddressYearsController.onPageLoad(index,CheckMode).url),
    ...
    val seqTradingTime = if (ua.get(AddressYearsId(index)).contains(false)) {
      Seq(
        answerOrAddRow(
          TradingTimeId(index),
          Messages("tradingTime.title", trusteeName),
          Some(controllers.trustees.company.address.routes.TradingTimeController.onPageLoad(index,CheckMode).url),
      ...
      case (Some(false), Some(true)) =>
        Seq(
          answerOrAddRow(
            ...
            Some(controllers.trustees.company.address.routes.EnterPreviousPostcodeController.onPageLoad(index,CheckMode).url),
