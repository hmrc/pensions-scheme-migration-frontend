...
class TrusteeCompanyContactDetailsCYAHelperForTwirl
  ...
  def contactDetailsRows(
    ...
    val trusteeName: String =
      CYAHelperForTwirl.getCompanyName(CompanyDetailsId(index))

    val rowsWithoutDynamicIndices = Seq(
      Some(answerOrAddRow(
        id = EnterEmailId(index),
        message = Message("messages__enterEmail_cya_label", trusteeName).resolve,
        url = Some(controllers.trustees.company.contacts.routes.EnterEmailController.onPageLoad(index, CheckMode).url),
        visuallyHiddenText = Some(Messages("messages__enterEmail__cya__visuallyHidden", trusteeName))
      )),
      Some(answerOrAddRow(
        id = EnterPhoneId(index),
        message = Message("messages__enterPhone_cya_label", trusteeName).resolve,
        url = Some(controllers.trustees.company.contacts.routes.EnterPhoneController.onPageLoad(index, CheckMode).url),
        visuallyHiddenText = Some(Messages("messages__enterPhone__cya__visuallyHidden", trusteeName))
      ))).flatten
