...
class EstablisherCompanyContactDetailsCYAHelperForTwirl
  ...
  def contactDetailsRows(
    ...
    val rowsWithoutDynamicIndices = Seq(
      Some(answerOrAddRow(
        ...
        url = Some(controllers.establishers.company.contact.routes.EnterEmailController.onPageLoad(index, CheckMode).url),
        visuallyHiddenText = Some(Messages("messages__enterEmail__cya__visuallyHidden", establisherName))
      )),
      Some(answerOrAddRow(
        id = EnterPhoneId(index),
        message = Message("messages__enterPhone_cya_label", establisherName).resolve,
        url = Some(controllers.establishers.company.contact.routes.EnterPhoneController.onPageLoad(index, CheckMode).url),
        visuallyHiddenText = Some(Messages("messages__enterPhone__cya__visuallyHidden", establisherName))
      ))).flatten
