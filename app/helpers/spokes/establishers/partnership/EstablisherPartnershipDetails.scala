...
import controllers.establishers.partnership.details.routes.WhatYouWillNeedController
...
case class EstablisherPartnershipDetails(
  index: Index,
  answers: UserAnswers
) extends Spoke {
  ...
  val linkKeyAndRoute: (String, String) = {
    if (completeFlag(answers).isDefined)
      (s"${messageKeyPrefix}changeLink", controllers.common.routes.CheckYourAnswersController.onPageLoad(index, entities.Establisher, entities.Partnership, entities.Details).url)
    else
  ...
  override def completeFlag(answers: UserAnswers): Option[Boolean] =
    answers.isEstablisherPartnershipDetailsComplete(index)
}
