...
case class EstablisherIndividualAddress(
  ...
  val linkKeyAndRoute: (String, String) = {
    if (completeFlag(answers).isDefined)
      (s"${messageKeyPrefix}changeLink", controllers.common.routes.CheckYourAnswersController.onPageLoad(index, entities.Establisher, entities.Individual, entities.Address).url)
    else
  ...
  override def completeFlag(answers: UserAnswers): Option[Boolean] =
    answers.isEstablisherIndividualAddressComplete(index)
}
