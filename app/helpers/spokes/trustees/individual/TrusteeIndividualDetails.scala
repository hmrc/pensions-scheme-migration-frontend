...
case class TrusteeIndividualDetails(
  ...
  val linkKeyAndRoute: (String, String) =
    if (completeFlag(answers).isDefined)
      (s"${messageKeyPrefix}changeLink", controllers.common.routes.CheckYourAnswersController.onPageLoad(index, entities.Trustee, entities.Individual, entities.Details).url)
    else
