...
case class TrusteeIndividualContactDetails(
index: Index,
  answers: UserAnswers
  ) extends Spoke {
  ...
  val linkKeyAndRoute: (String, String) = {
    if (completeFlag(answers).isDefined)
      (s"${messageKeyPrefix}changeLink", controllers.common.routes.CheckYourAnswersController.onPageLoad(index, entities.Trustee, entities.Individual, entities.Contacts).url)
    else
  ...
  override def completeFlag(answers: UserAnswers): Option[Boolean] =
    answers.isTrusteeIndividualContactDetailsComplete(index)
}
