...
package helpers.spokes.trustees.partnership

import controllers.trustees.partnership.address.routes.WhatYouWillNeedController
...
case class TrusteePartnershipAddress(
  index: Index,
  answers: UserAnswers
) extends Spoke {
  ...
  val linkKeyAndRoute: (String, String) = {
    if (completeFlag(answers).isDefined)
      (s"${messageKeyPrefix}changeLink", controllers.common.routes.CheckYourAnswersController.onPageLoad(index, entities.Trustee, entities.Partnership, entities.Address).url)
    else
  ...
  override def completeFlag(answers: UserAnswers): Option[Boolean] =
    answers.isTrusteePartnershipAddressComplete(index)
}
