...
import controllers.trustees.company.address.routes.WhatYouWillNeedController
...
case class TrusteeCompanyAddress(
  index: Index,
  answers: UserAnswers
) extends Spoke {
  ...
  val linkKeyAndRoute: (String, String) = {
    if (completeFlag(answers).isDefined)
      (s"${messageKeyPrefix}changeLink", controllers.common.routes.CheckYourAnswersController.onPageLoad(index, entities.Trustee, entities.Company, entities.Address).url)
    else
  ...
  override def completeFlag(answers: UserAnswers): Option[Boolean] =
    answers.isTrusteeCompanyAddressComplete(index)
}
