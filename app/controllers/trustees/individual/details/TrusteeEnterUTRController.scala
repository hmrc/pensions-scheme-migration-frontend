    ...
    if (mode == CheckMode) {
      ...
      for(director <- directors) {
        if (!director.isDeleted)
          updatedUserAnswers = updatedUserAnswers.get.set(DirectorEnterUTRId(director.mainIndex.get, director.index), value)
      }
    }
    val finalUpdatedUserAnswers = updatedUserAnswers.get.set(TrusteeUTRId(index), value)
