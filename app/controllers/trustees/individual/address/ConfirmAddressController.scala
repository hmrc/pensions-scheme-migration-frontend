    ...
    if (mode == CheckMode) {
      ...
      for (director <- directors) {
        if (!director.isDeleted)
          updatedUserAnswers = updatedUserAnswers.get.set(Director.AddressId(director.mainIndex.get, director.index), value)
      }
    }
  val finalUpdatedUserAnswers = updatedUserAnswers.get.set(AddressId(index), value)
