...
//
//  override protected val pageTitleEntityTypeMessageKey: Option[String] = Some("establisherEntityTypePartnership")
...
//          get(Some(schemeName), partnershipDetails.partnershipName, PreviousAddressId(index), AddressConfiguration.PostcodeFirst)
//      }
//    }
//
...
//    (authenticate andThen getData andThen requireData).async { implicit request =>
//      (PartnershipDetailsId(index) and SchemeNameId).retrieve.right.map { case partnershipDetails ~ schemeName =>
//        post(Some(schemeName), partnershipDetails.partnershipName, PreviousAddressId(index), AddressConfiguration.PostcodeFirst)
//      }
//    }
//}
