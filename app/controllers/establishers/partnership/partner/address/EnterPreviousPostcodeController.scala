...
class EnterPreviousPostcodeController @Inject()(val appConfig: AppConfig,
  ...
  def onSubmit(establisherIndex: Index, partnerIndex: Index, mode: Mode): Action[AnyContent] = (authenticate andThen getData andThen requireData).async{
    implicit request =>
      retrieve(SchemeNameId) { schemeName =>
        post(getFormToJson(schemeName, establisherIndex, partnerIndex, mode), EnterPreviousPostCodeId(establisherIndex, partnerIndex), "establisherEnterPostcode.invalid", Some(mode))
      }
  }
  ...
  def getFormToJson(schemeName:String, establisherIndex: Index, partnerIndex: Index, mode: Mode)(implicit request:DataRequest[AnyContent]): Form[String] => JsObject = {
    form => {
      ...
      Json.obj(
        ...
        "entityName" -> name,
        "form" -> form,
        "enterManuallyUrl" -> controllers.establishers.partnership.partner.address.routes.ConfirmPreviousAddressController.onPageLoad(establisherIndex, partnerIndex, mode).url,
