...
class EstablisherNoUTRReasonController @Inject()(val messagesApi: MessagesApi,
    ...
    (authenticate andThen getData andThen requireData()).async {
      implicit request =>
        SchemeNameId.retrieve.map {
          schemeName =>
            common.get(
              pageTitle     = Messages("messages__whyNoUTR", Messages("messages__individual")),
              pageHeading     = Messages("messages__whyNoUTR", name(index)),
              isPageHeading = true,
              id            = EstablisherNoUTRReasonId(index),
              form          = form(index),
              schemeName    = schemeName,
              submitUrl     = routes.EstablisherNoUTRReasonController.onSubmit(index, mode)
            )
        }
    }
