...
class CheckYourAnswersController @Inject()(
                                            ...
                                            cyaHelper: EstablisherAddressCYAHelper,
    ...
    (authenticate andThen getData andThen requireData()).async {
      implicit request =>
        val ctx = Json.obj(
          ...
          "submitUrl"  -> controllers.establishers.individual.routes.SpokeTaskListController.onPageLoad(index).url
        )

        val template = TwirlMigration.duoTemplate(
          renderer.render("check-your-answers.njk", ctx),
          checkYourAnswersView(
            controllers.establishers.individual.routes.SpokeTaskListController.onPageLoad(index).url,
            CYAHelper.getAnswer(SchemeNameId)(request.userAnswers, implicitly),
            TwirlMigration.summaryListRow(cyaHelper.rows(index))
          )
        )
